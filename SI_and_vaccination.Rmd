...
library(kableExtra)
library(shiny)
library(ecoevoapps) # now only used for 
...
make_rectangles <- function(S_scale=1, # looks good: min 0.1 and max 3
                            ...
                            b_scale=0.5, # transmission (S -> I): min 0.1, max 2.2
                            c_scale=0.5, # recovery (I -> S): min 0.1, max 2.2
                            p_scale=0.5, # vaccination policy (S -> V): min 0.1, max 2.2
                            w_scale=0.5, # vaccination waning (V -> S): min 0.1, max 2.2
                            m_scale=0.5 # modifier of transmission rate (V -> I): min 0.1, max 2.2
                            ){
  # check if vaccination is enabled
  vaccine <- (V_scale | p_scale) # at least one must be non-zero
  ...
  if (b_scale > 0){
    output <- output +
            # plot the transmission arrow
              ...
              # weight of arrow depends on b parameter
                         linewidth=b_scale) +
    ...
    output <- output +
            # the recovery arrow
              geom_curve(aes(xend = rect_coords["Susceptible", "x_center"]+0.75,
                         ...
                         curvature = -0.5, lineend = "butt",
                         arrow = arrow(length = unit(0.04, "npc")),
              # weight of arrow depends on c parameter
                         linewidth=c_scale) +
            # add recovery label
              geom_text(aes(x = 0.05 + rect_coords["Susceptible", "x_center"] + (rect_coords["Infected", "x_center"] - rect_coords["Susceptible", "x_center"])/2,
    ...
    output <- output +
            # define the vaccination rate arrow
              geom_curve(aes(x = rect_coords["Susceptible", "x_center"]+0.15,
                         ...
                         arrow = arrow(length = unit(0.04, "npc")),
            # weight of arrow depends on p parameter
                         linewidth=p_scale) +
            # add vaccination rate label
              geom_text(aes(x=1.75, y=2.3, label = "p"), size = 5)
  }
  
  # if vaccine can wane, add the waning rate arrow
  if (vaccine & (w_scale > 0)) { # to prevent extraneous arrows or labels, need to check that vaccine is also TRUE
    output <- output +
            # define the waning rate arrow
              geom_curve(aes(xend = rect_coords["Susceptible", "x_center"]-0.05,
                         ...
                         curvature = -0.25, lineend = "butt",
                         arrow = arrow(length = unit(0.04, "npc")),
            # weight of arrow depends on w parameter
                         linewidth=w_scale) +
            # add vaccination rate label
              geom_text(aes(x=0.6, y=2.3, label = "w"), size = 5)
  }
  
  #if vaccine is not perfectly effective, add modified transmission arrows
  if (vaccine & (m_scale > 0)){ # to prevent extraneous arrows or labels, need to check that vaccine is also TRUE
    output <- output +
          # finally define the vaccine-modified transmission rate arrow
              geom_curve(aes(x = rect_coords["Vaccinated", "x_center"]+0.7,
                         ...
                         arrow = arrow(length = unit(0.04, "npc")),
              # weight of arrow depends on m*b parameters
                         linewidth=m_scale) +
              # add vaccination rate label
              geom_text(aes(x=4.27, y=2.05, label = "m*b"), size = 5)
  }
...
#print(make_rectangles(S_scale=1, I_scale=1, V_scale=0, p_scale=0))
```
...
#' @param params vector of mu (natural birth and death rate),
#' beta (transmission rate), gamma (recovery rate), theta (vaccine waning
#' rate), sigma (ability of vaccinated to produce new infections),
...
SVI <- function(time,init,params) {
  with (as.list(c(time,init,params)), {
    ...
    dS_dt = mu*(S + V + I) + theta*V + gamma*I - (beta*I/(S + V + I))*S - (mu + phi)*S
    dV_dt = phi*S - (sigma*beta*I/(S + V + I))*V - (mu + theta)*V
    dI_dt = (beta*I/(S + V + I))*S + (sigma*beta*I/(S + V + I))*V - (mu + gamma)*I
...
plot_SVI_time <- function(sim_df, params, title_text=NA) {
  ...
  finite_plot <- ggplot(sim_df_long) +
                  ...
                  geom_line(aes(x = time, y = value, color = Group), linewidth = 2, alpha=0.8) +
                  scale_colour_viridis_d() +
                  ylab("Population fraction") +
                  scale_y_continuous(limits = c(0,1), expand = c(0, 0)) + # important to set expand to 0,0 otherwise there's extra padding that makes it look like things aren't going to 0 or 1
  ...
  inf_plot <- ggplot() +
                ...
                scale_x_continuous(limits=c(0.5,1.5), breaks = c(1), labels = "Inf") +
                scale_colour_viridis_d() +
...
get_bifurcation <- function(init, params){
  ...
  R0_phi <- R0 * ((mu + theta + sigma*phi) / (mu + theta + phi))
  R0_phi <- unname(R0_phi)
  
  # summarize model outcome by examining inequalities (see pg 192)
  inequality1 <- (mu + theta + sigma*phi)^2 < (mu + gamma) * sigma * (1 - sigma) * phi
  inequality2 <- (mu + gamma - ((mu + theta + sigma*phi)/(sigma)) + (2/sigma)*sqrt((mu + gamma)*sigma*(1-sigma)*phi)) < beta & beta < (mu + gamma)*((mu + theta + phi)/(mu + theta + sigma*phi))
  ...
  if (R0 < 1) {
    ...
    V_star <- ifelse(phi + theta > 0, # check for potential undefined values
                     phi / (phi + theta),
                     0)
  } else if (is.nan(R0_phi)) { # when mu, theta, and phi are 0 then R0_phi is undefined
    ...
    behav <- "system always goes to endemic disease equilibrium."
    # see pg 189
    I_star <- (1 - ((1/R0)*(1 + ((phi)/(mu + theta))))) * sum(init)
    V_star <- ifelse(theta + theta + sigma*beta > 0, # check for potential undefined values
                     (phi - phi*I_star) / (phi + theta + sigma*beta*I_star),
                     0)
  } else if (all(inequality1, inequality2)){
    behav <- "disease-free & endemic equilibria are both locally stable: system depends on initial conditions."
    I_star <- "it depends."
    V_star <- "it depends."
    S_star <- "it depends."
  ...
  if(!exists("S_star")) S_star <- 1 - I_star - V_star
...
sidebarLayout(
  sidebarPanel(
    ...
    sliderInput("beta", label = "b (transmission rate):",
    ...
    sliderInput("gamma", label = "c (recovery rate):",
                min = 0, max = 3, value = .5, step = 0.01),
    
    sliderInput("theta", label = "w (vaccine waning rate):",
                min = 0, max = 3, value = .01, step = 0.01),
    
    sliderInput("sigma", label = "m (vaccine-modified transmission):",
                min = 0, max = 1, value = .02, step = 0.02),
    
    sliderInput("phi", label = "p (vaccination rate):",
...
init_user <- reactive({c(S = input$S0, V = input$V0, I = input$I0)})
init_rareI <- c(S = 1 - 10^-12, V = 0, I = 10^-12)
init_commonI <- c(S = 10^-4, V = 0, I = 1 - 10^-4)

# Set the parameter values
params <- reactive({c(mu = 0,
                      beta = input$beta, 
                      gamma = input$gamma,
                      theta = input$theta,
                      sigma = input$sigma,
...
schematic_plot <- reactive({
  make_rectangles(#S_scale = ifelse(input$S0 > 0, input$S0 * 3, 0.05),
          ...
          # remove the V rectangle only when there's NO vaccination and NO vaccinated at time=0
                  V_scale = ifelse((input$V0 == 0) & (input$phi == 0), 0, 1),
                  b_scale = ifelse(input$beta > 0, (input$beta/3)*2.1 + 0.1, 0),
                  c_scale = ifelse(input$gamma > 0, (input$gamma/3)*2.1 + 0.1, 0),
                  p_scale = ifelse(input$phi > 0, (input$phi/0.5)*2.1 + 0.1, 0),
                  w_scale = ifelse(input$theta > 0, (input$theta/3)*2.1 + 0.1, 0),
          # thickness of the m*b arrow depends on BOTH m and b together!
                  m_scale = ifelse((input$sigma > 0) & (input$beta > 0),
...
par_dfe <- c(mu = 0, beta = 0.27, gamma = 0.5, theta = 0.01, sigma = 0.02, phi = 0)

# disease-free as a result of vaccination effort
par_vacc <- c(mu = 0.01, beta = 3.27, gamma = 0.5, theta = 0.01, sigma = 0.02, phi = 0.2)

# hysteresis
par_hyst <- c(mu = .01, beta = 3.27, gamma = 0.5, theta = 0.01, sigma = 0.02, phi = 0.125)

# hysteresis WITHOUT birth&death (mu = 0)
par_hyst_mu0 <- c(mu = 0, beta = 3.27, gamma = 0.306, theta = 0.01, sigma = 0.02, phi = 0.125)

# I think this hysteresis WITHOUT birth&death (mu) and WITHOUT wanning (theta) ?
test_this <- c(mu = 0, beta = 3.27, gamma = 0.5, theta = 0, sigma = 0.02, phi = 0.01)
...
```{r testings, echo=FALSE}
test_init <- c(S=0.9, V=0, I=0.1)
test_pars <- c(mu=0, beta=1.93, gamma=1.93, theta=0, sigma=0, phi=0)

get_bifurcation(init = test_init, params = test_pars)
test_sim <- data.frame(run_SVI_model(time = seq(1, 200, by=2), init = test_init, params = test_pars))
plot_SVI_time(sim_df = test_sim, params = test_pars)

init_rareI <- c(S = 1 - 10^-12, V = 0, I = 10^-12)
get_bifurcation(init = init_rareI, params = test_pars)
test_sim <- data.frame(run_SVI_model(time = seq(1, 200, by=2), init = init_rareI, params = test_pars))
plot_SVI_time(sim_df = test_sim, params = test_pars)

init_commonI <- c(S = 10^-4, V = 0, I = 1 - 10^-4)
get_bifurcation(init = init_commonI, params = test_pars)
test_sim <- data.frame(run_SVI_model(time = seq(1, 200, by=2), init = init_commonI, params = test_pars))
plot_SVI_time(sim_df = test_sim, params = test_pars)
```
